    ...
    switch (pathname) {
        ...
        case '/client.html':
            fs.readFile(clientPath, (err, data) => {
                if (err) return respondJSON(request, response, 500, 'Internal Server Error');
        ...
        case '/pokemon':
            if (request.method === 'HEAD') {
                ...
                if (type) filteredData = filteredData.filter(p => p.type.some(t => t.toLowerCase() === type.toLowerCase()));
                if (weakness) filteredData = filteredData.filter(p => p.weaknesses.some(w => w.toLowerCase() === weakness.toLowerCase()));
            ...
            if (request.method === 'POST') {
                ...
                request.on('end', () => {
                    ...
                    if (contentType === 'application/json') {
                        try { newPokemon = JSON.parse(body); }
                        catch (err) { return respondJSON(request, response, 400, 'Invalid JSON', 'badRequest'); }
                    } else if (contentType === 'application/x-www-form-urlencoded') {
                        const params = new URLSearchParams(body);
                        newPokemon = { //list of json required data based off pokedex.json
                        ...
                        return respondJSON(request, response, 400, 'Unsupported Content-Type', 'badRequest');
                    }
        ...
        case '/editPokemon':
            if (request.method === 'POST') {
                ...
                request.on('end', () => {
                    ...
                    if (contentType === 'application/json') {
                        try { updatedPokemon = JSON.parse(body); }
                        catch (err) { return respondJSON(request, response, 400, 'Invalid JSON', 'badRequest'); }
                    } else if (contentType === 'application/x-www-form-urlencoded') {
                        const params = new URLSearchParams(body);
                        updatedPokemon = { //list of json required data based off pokedex.json
                            id: parseInt(params.get('id')),
                            num: params.get('num'),
                            name: params.get('name'),
                            img: params.get('img'),
                    ...
                    if (index === -1) return respondJSON(request, response, 404, 'Pokemon not found', 'notFound'); //404

                    // check if data is actually changed or not
                    const existing = pokemonData[index];
                    const isUnchanged = JSON.stringify(existing) === JSON.stringify(updatedPokemon);

                    if (isUnchanged) return respondHead(response, 204); // 204 if nothing is changed /
